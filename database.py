...
def write_to_csv(input_data: list[str], input_file_name: str = 'bp.csv'):
    try:
        ...
        #logger_file.log_info(f'Record {input_data[0]};{input_data[1]};{input_data[2]};{input_data[3]}  was written to CSV file', 'write_to_text')
    ...
    try:
        with open(input_file_name, "a", encoding='utf-8') as data:
            data.write(
                ...
                f'\n{input_data[2]}'
                f'\n{input_data[3]}\n\n')
        #logger_file.log_info(f'Record {input_data[0]};{input_data[1]};{input_data[2]};{input_data[3]} was written to text file', 'write_to_text')
    except Exception as e:
        logging.debug(e)
